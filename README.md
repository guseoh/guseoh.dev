...
- 글 상세: 메타데이터, 태그, TOC, 관련 글, 읽기 진행률, 마지막 읽은 위치 복원, 이미지 lightbox, 코드블록 파일명/줄번호/줄강조, Copy 버튼, 이전/다음 글
...
slug: "board/post-detail-performance"
aliases: []
commentKey: "/blog/board/post-detail-performance/"
category: Board
...
- `slug`는 공개 URL을 결정하는 고정 값입니다. 파일이나 폴더를 옮겨도 기존 글의 `slug`는 바꾸지 않습니다.
- `aliases`에는 과거 URL을 `/blog/.../` 형태로 적습니다. alias 페이지는 canonical과 자동 이동 안내를 제공합니다.
- `commentKey`는 Utterances 댓글 Issue 연결용 고정 값입니다. 기존 댓글을 유지하려면 처음 발행한 URL 값을 보존합니다.
- `category`는 글 분류, `tags`는 검색과 필터용 키워드입니다.
- `testedWith`에는 실제로 검증한 Java, Spring Boot 등 기술 버전만 선택적으로 적습니다.
...
7. 작성 중에는 `draft: true`를 유지하고 발행 전에 `draft: false`로 변경합니다.
...
- 이미지 lightbox 확대 보기에서 제외할 작은 아이콘, 배지, 장식 이미지는 alt나 title에 `{no-lightbox}`를 붙입니다. 렌더링 시 표시는 제거됩니다.
...
코드 블록에는 선택적으로 파일명, 줄번호, 줄강조를 지정할 수 있습니다. 줄강조 범위가 실제 코드 줄 수를 넘으면 빌드 단계에서 오류로 처리됩니다.

````md
```sql title="flyway_schema_history.sql" showLineNumbers {2,4-5}
SELECT *
...
Markdown 표 바로 앞에 `table-caption` 주석을 두면 접근 가능한 `<caption>`으로 렌더링됩니다.

```md
<!-- table-caption: Flyway 마이그레이션 파일명 구성 -->
| 구성 | 의미 |
| --- | --- |
| V | Versioned Migration 접두사 |
```

외부 링크 점검은 `npm run links:check`로 실행합니다. 이 명령은 명확한 404/410 링크만 실패로 처리하고, 403/429/5xx/네트워크 오류는 경고로 구분합니다. 동일한 점검은 `.github/workflows/check-links.yml`에서 주 1회 수동 배포와 분리해 실행됩니다.
...
- `heroImage`가 없거나 기본 이미지이면 빌드 시 생성되는 `/og/<slug>.svg` 글별 OG 이미지를 사용합니다.
- 글별 OG 이미지는 제목, 카테고리, 대표 태그, 사이트명, 작성자명을 포함합니다.
...
Smoke test는 URL 상태와 Content-Type 외에도 검색 인덱스의 공개 글 개수, `/blog/`의 배포 검증용 `data-*` 값, 최신 글의 sitemap/RSS 포함 여부, draft 글 제외 여부, 배포 SHA를 함께 검사합니다.
